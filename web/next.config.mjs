...
const nextConfig = {
  ...
  // Bare /mascot serves the static character sheet. Router-level rewrite:
  // a pure internal path mapping, immune to the forwarded-proto/origin
  // mismatch that 500s middleware rewrites behind the cloudflared tunnel
  // (x-forwarded-proto: https + host localhost:3001 = "external" rewrite).
  async rewrites() {
    return {
      beforeFiles: [
