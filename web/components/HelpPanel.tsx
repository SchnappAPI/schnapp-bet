  ...
  return (
    <>
      <button
        ...
        className="text-fg-disabled hover:text-fg-subtle text-xs px-1.5 py-0.5 rounded border border-transparent hover:border-border transition-colors leading-none"
      ...
      {open && (
        <>
          ...
          />
          <div className="fixed inset-y-0 right-0 z-50 w-full max-w-sm bg-canvas border-l border-border shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-border">
              <span className="text-sm font-semibold text-fg-muted">{content.title}</span>
              <button
                onClick={() => setOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              {content.sections.map((section) => (
                <div key={section.heading}>
                  <h3 className="text-xs font-semibold text-fg-subtle uppercase tracking-wider mb-1.5">
                    {section.heading}
                  </h3>
                  {section.body && (
                    <p className="text-xs text-fg-subtle leading-relaxed mb-2">
                      {section.body}
                    </p>
                  )}
                  {section.glossary && (
                    <div className="space-y-1.5">
                      {section.glossary.map(({ term, def }) => (
                        <div key={term} className="flex gap-2 text-xs">
                          <span className="text-fg-muted font-medium whitespace-nowrap w-16 flex-none">
                            {term}
                          </span>
                          <span className="text-fg-subtle leading-relaxed">{def}</span>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div className="px-5 py-3 border-t border-border">
              <p className="text-xs text-fg-disabled">schnapp.bet</p>
            </div>
          </div>
        </>
      )}
