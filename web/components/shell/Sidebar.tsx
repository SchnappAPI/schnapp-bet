...
export function Sidebar({ isAdmin = false, variant = 'rail', onNavigate }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Brand */}
      <div className={cn('flex h-12 items-center border-b border-border', collapsed ? 'justify-center px-0' : 'px-3')}>
        <Link href="/" className="flex items-center gap-2 text-fg">
          <span aria-hidden className="inline-flex h-5 w-5 items-center justify-center rounded-sm bg-brand text-white text-[11px] font-bold font-mono">
            ▣
          </span>
          {!collapsed && (
      ...
      <nav className="flex-1 overflow-y-auto py-2" onClick={onLinkActivate}>
        <div className="px-2">
          <SidebarLink href="/" icon={<Home size={14} />} label="Today" collapsed={collapsed} />
        </div>
        ...
        <div className="px-2">
          {/* Single entry — view switching (Vs / EV / Projections / Players /
              Pitchers) lives in the in-page tab strip, matching the NBA pattern.
              Sidebar is for top-level navigation; sub-views are tabs inside the
              page. */}
          <SidebarLink
            href="/mlb"
        ...
        <SectionLabel collapsed={collapsed}>OTHER</SectionLabel>
        <div className="px-2">
          <SidebarLink
            href="/nfl"
            ...
            collapsed={collapsed}
            disabled
            trailing={!collapsed && <Chip tone="neutral" size="xs">soon</Chip>}
          />
          <SidebarLink href="/lol" icon={<Gauge size={14} className="text-sport-lol" />} label="LoL" collapsed={collapsed} />
        </div>

        <div className="my-2 mx-3 border-t border-border-subtle" />

        <div className="px-2">
          <SidebarLink href="/transparency" icon={<BarChart3 size={14} />} label="Transparency" collapsed={collapsed} />
          {isAdmin && (
            <SidebarLink href="/admin" icon={<Lock size={14} />} label="Admin" collapsed={collapsed} />
          )}
        </div>
      </nav>
      </Suspense>
...
function SectionLabel({ children, collapsed }: { children: React.ReactNode; collapsed: boolean }) {
  if (collapsed) return <div className="my-2 mx-3 border-t border-border-subtle" />;
  return (
