...
export default function RosterTable({ gameId, selectedDate }: Props) {
  ...
  if (loading) return <div className="text-sm text-fg-subtle py-4">Loading roster...</div>;
  if (error)   return <div className="text-sm text-neg py-4">Error: {error}</div>;
  if (roster.length === 0) return <div className="text-sm text-fg-subtle py-4">No lineup data available yet.</div>;
  ...
  function teamBadge(abbr: string): { label: string; cls: string } {
    ...
    if (hasConfirmed) return { label: 'Confirmed', cls: 'text-pos border-border' };
    if (hasProjected) return { label: 'Projected', cls: 'text-warn border-border' };
    return { label: 'Expected', cls: 'text-fg-disabled border-border' };
  }
  ...
  const renderPlayer = (p: RosterRow, isStarter: boolean, dimmed = false) => (
    <tr key={p.playerName} className={['border-b border-border', dimmed ? 'opacity-40' : ''].join(' ')}>
        ...
        {p.playerId != null ? (
          <Link
            ...
            className={[
              ...
              dimmed ? 'text-fg-subtle' : isStarter ? 'text-fg' : 'text-fg-muted',
            ].join(' ')}
          >
            {p.playerName}
          </Link>
        ) : (
          <span className={dimmed ? 'text-fg-subtle' : isStarter ? 'text-fg' : 'text-fg-muted'}>
            {p.playerName}
          </span>
        )}
      </td>
      <td className="py-1.5 pr-2 text-fg-subtle text-xs">{p.position ?? ''}</td>
      <td className="py-1.5 text-right">
        {isStarter && (
          <span className="text-xs bg-brand-muted text-brand px-1 rounded">S</span>
        )}
      </td>
    </tr>
  );
  ...
  const sectionHeader = (label: string) => (
    <tr>
      <td colSpan={3} className="pt-3 pb-0.5 text-xs text-fg-disabled font-semibold uppercase tracking-wider">
        {label}
      </td>
    </tr>
  );
