...
function evPctColor(evPct: number | null): string {
  if (evPct == null) return 'text-gray-600';
  if (evPct > 8)    return 'text-green-400';
  if (evPct >= 0)   return 'text-yellow-400';
  return 'text-gray-500';
}
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-3 mb-4 text-xs text-gray-600">
        <span>Odds color: <span className="text-green-400">green</span>=EV%&gt;8 <span className="text-yellow-400">yellow</span>=EV%&ge;0 <span className="text-gray-500">gray</span>=EV%&lt;0 or no data</span>
                        ...
                        {players.map((player) => (
                          <tr key={player.playerId} className="border-t border-gray-900 hover:bg-gray-900/30 transition-colors">
                            ...
                            {cols.map((line) => {
                              ...
                              const oddsText = (
                                <span className={`tabular-nums ${evPctColor(cell.evPct)}`}>
                                  {fmtOdds(cell.price)}
                                </span>
                              );
                              ...
                              return (
                                <td key={line} className={`py-1.5 px-2 text-right ${bg} whitespace-nowrap`}>
                                  <span className="inline-flex items-center gap-0.5 justify-end">
                                    {cell.evPct != null && (
                                      <span className={`text-[9px] opacity-70 ${evPctColor(cell.evPct)}`}>
                                        {cell.evPct > 0 ? '+' : ''}{cell.evPct.toFixed(0)}%
                                      </span>
                                    )}
                                    {resultBadge ?? (
                                      cell.link ? (
