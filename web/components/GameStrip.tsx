        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className={`text-xs font-semibold ${isLive ? 'text-neg' : 'text-fg-subtle'}`}>
                {statusLabel}
              </span>
            </div>
            ...
            {(isLive || isFinal) && hasScore ? (
              ...
              <div className="text-sm font-bold flex items-center gap-1.5">
                <span className={!homeLeads ? 'text-fg' : 'text-fg-subtle'}>
                  {game.awayScore}
                </span>
                <span className="text-xs text-fg-disabled">{game.awayTeamAbbr}</span>
                <span className="text-fg-disabled text-xs">@</span>
                <span className="text-xs text-fg-disabled">{game.homeTeamAbbr}</span>
                <span className={homeLeads ? 'text-fg' : 'text-fg-subtle'}>
                  {game.homeScore}
                </span>
              </div>
              ...
              <div className="text-sm font-bold">
                {game.awayTeamAbbr} <span className="text-fg-subtle">@</span> {game.homeTeamAbbr}
              </div>
            )}
            ...
            {!(isLive || isFinal) && (
              <div className="text-xs text-fg-subtle mt-1">
                ...
                {spreadLabel == null && game.total == null && <span className="text-fg-disabled">No line</span>}
              </div>
            )}
          </button>
        );
