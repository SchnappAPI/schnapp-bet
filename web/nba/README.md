...
The three routes that call Flask (`/api/games`, `/api/scoreboard`, `/api/live-boxscore`) read `process.env.RUNNER_URL` rather than a hardcoded hostname. Production sets `RUNNER_URL=https://mac-flask.schnapp.bet` in the `bet.schnapp.web-prod.plist`; that hostname is Cloudflare-proxied through the `schnapp-mac` tunnel to the Mac Flask runner (`bet.schnapp.flask` launchd agent on `127.0.0.1:5000`). Dev points `RUNNER_URL` at `http://127.0.0.1:5000`. The literal default of `https://live.schnapp.bet` in the route source is dead code — that DNS record was deleted 2026-05-01. Never hardcode hostnames or IPs in web routes; always read `RUNNER_URL`. The `X-Runner-Key` header (env: `RUNNER_API_KEY`) is still required on `/scoreboard` and `/boxscore`; `/ping` is open.
...
- `/api/ping` - anonymous. `SELECT 1` via `ping()`. Route retained for future Uptime Robot re-enablement and ad-hoc health checks (monitor is currently paused)
- `/api/games?date=&sport=nba` - reads the game list from `nba.schedule` via `getGames()` for any date. For today only, overlays CDN scoreboard data (status + scores) onto games already present in the DB via `${RUNNER_URL}/scoreboard`. CDN never drives the game list. Falls back to DB-only if Flask is unreachable
- `/api/scoreboard` - thin passthrough to Flask `/scoreboard` via `${RUNNER_URL}`
...
- `/api/live-boxscore?gameId=` - calls Flask `/boxscore` via `${RUNNER_URL}`
...
- All web routes that call Flask read `process.env.RUNNER_URL`. Production sets it to `https://mac-flask.schnapp.bet` (Cloudflare-proxied via the `schnapp-mac` tunnel to the Mac Flask runner). Never hardcode hostnames or IPs
...
- `/api/grades` date predicate is `WHERE dg.grade_date = @gradeDate` (sargable). Do not wrap `dg.grade_date` in `CONVERT()`; the wrapped form is slow and times out in production
