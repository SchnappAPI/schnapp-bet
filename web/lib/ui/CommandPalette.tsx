...
interface SearchResponse {
  players: Array<{ id: number; name: string; team_abbr: string | null; sport: 'nba' | 'mlb' }>;
  games: Array<{ id: string; label: string; sport: 'nba' | 'mlb'; date: string }>;
}
...
export function CommandPalette({ children, placeholder = 'Search players, games, actions…' }: CommandPaletteProps) {
  ...
  const swrKey = open && trimmed.length >= 2
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 border-b border-border px-3">
          ...
          />
          <kbd className="font-mono text-[10px] px-1 py-0.5 border border-border-strong rounded-sm bg-inset text-fg-subtle">ESC</kbd>
        </div>
            ...
            <CommandGroup heading="Players">
              {data.players.map((p) => (
                <CommandItem
                  ...
                  onSelect={() => {
                    setOpen(false);
                    window.location.href = p.sport === 'nba' ? `/nba/player/${p.id}` : `/mlb/player/${p.id}`;
                  }}
                >
            ...
            <CommandGroup heading="Games">
              {data.games.map((g) => (
                <CommandItem
                  ...
                  onSelect={() => {
                    setOpen(false);
                    window.location.href = g.sport === 'nba'
                      ? `/nba?gameId=${encodeURIComponent(g.id)}&date=${g.date}`
                      : `/mlb?gameId=${encodeURIComponent(g.id)}&view=game&date=${g.date}`;
                  }}
                >
