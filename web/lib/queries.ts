...
// ---------------------------------------------------------------------------
// Schema cache — INFORMATION_SCHEMA column-existence checks at module load.
// daily_grades and upcoming_player_props are stable enough that we only need
// to look once per process. A cache miss falls back to the DB; the cache is
// populated lazily on first call. Subsequent calls hit memory only.
// ---------------------------------------------------------------------------

const GRADES_TRACKED_COLS = [
...
let cachedGradesCols: Set<string> | null = null;
let cachedHasLinkCol: boolean | null = null;
let schemaInitPromise: Promise<void> | null = null;

async function ensureSchemaCache(): Promise<void> {
  if (cachedGradesCols !== null && cachedHasLinkCol !== null) return;
  if (schemaInitPromise) return schemaInitPromise;

  schemaInitPromise = (async () => {
    const pool = await getPool();
    const colsResult = await pool.request().query<{ column_name: string }>(
      `SELECT column_name
    ...
    cachedGradesCols = new Set(colsResult.recordset.map((r) => r.column_name));

    const linkResult = await pool.request().query<{ column_name: string }>(
      `SELECT column_name
       FROM information_schema.columns
    ...
    cachedHasLinkCol = linkResult.recordset.length > 0;
  })();
...
export type GradesSport = 'nba' | 'mlb' | 'all';

export async function getGrades(
  gradeDate: string,
  gameId: string | null,
  sport: GradesSport = 'nba'
): Promise<GradeRow[]> {
  await ensureSchemaCache();
  const pool = await getPool();

  const existingCols = cachedGradesCols ?? new Set<string>();
  const hasLinkCol   = cachedHasLinkCol ?? false;
  ...
  const result = await req.query<GradeRow>(
    `SELECT
       ...
       ${sel('model_prob',       'modelProb')},
       ${sel('implied_prob',     'impliedProb')},
       ${sel('ev_pct',           'evPct')},
       ${sel('outcome',          'outcome')},
     ...
     WHERE dg.grade_date = @gradeDate
       ${sport === 'nba' ? "AND dg.model_version NOT LIKE 'mlb%'" : ''}
       ${sport === 'mlb' ? "AND dg.model_version LIKE 'mlb%'" : ''}
       AND (s.game_date IS NULL OR CONVERT(VARCHAR(10), s.game_date, 120) = @gradeDate)
     ${gameFilter}
     ORDER BY COALESCE(dg.ev_pct, dg.composite_grade, dg.grade) DESC`
  );
  return result.recordset;
}
