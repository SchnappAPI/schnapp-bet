...
function oddsColor(price: number | null): string {
  if (price == null) return 'text-fg-disabled';
  if (price > 0) return 'text-fg-subtle';
  if (price >= -115) return 'text-fg-subtle';
  return 'text-fg-subtle';
}
...
function rankColor(rank: number): string {
  if (rank <= 10) return 'text-pos';
  if (rank <= 20) return 'text-warn';
  return 'text-neg';
}
...
function SignalLegend() {
  ...
  return (
    <div className="flex flex-wrap gap-2 px-4 py-2 border-b border-border">
      {items.map(({ type, desc }) => (
        <span key={type} className="flex items-center gap-1 text-xs text-fg-subtle" title={desc}>
...
export default function GradesPageInner() {
  ...
  function sortIndicator(key: SortKey) {
    if (sortKey !== key) return <span className="text-fg-disabled ml-0.5">&#8597;</span>;
    return sortDir === 'desc'
  ...
  function SortTh({ col, label, title, right }: { col: SortKey; label: string; title?: string; right?: boolean }) {
    return (
      <th
        className={`py-1.5 ${right ? 'px-2 text-right' : 'pr-3 text-left'} font-medium cursor-pointer select-none whitespace-nowrap hover:text-fg-muted transition-colors`}
        title={title}
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="px-4 py-3 border-b border-border flex items-center gap-3 flex-wrap">
        <Link href={backHref} className="text-fg-subtle hover:text-fg-muted text-sm">
          &#8592; Games
        </Link>
        <span className="text-sm font-semibold text-fg-subtle uppercase tracking-wider">
          At a Glance
        </span>
        <span className="text-xs text-fg-disabled">{gradeDate}</span>
        <HelpButton page={viewMode === 'matrix' ? 'grades-matrix' : 'grades-list'} />

        {hasLiveGames && (
          <span className="text-xs text-pos font-medium">&#9679; Live</span>
        )}
          ...
          <div className="flex rounded border border-border overflow-hidden text-xs font-medium">
            <button
              ...
              className={`px-3 py-1 transition-colors ${
                outcomeFilter === 'Over' ? 'bg-surface-hover text-fg' : 'text-fg-subtle hover:text-fg-muted'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 transition-colors border-l border-border ${
                outcomeFilter === 'Under' ? 'bg-surface-hover text-fg' : 'text-fg-subtle hover:text-fg-muted'
              }`}
            >
        ...
        {!loading && !error && grades.length > 0 && (
          <div className="flex rounded border border-border overflow-hidden text-xs font-medium">
            {(['all', 'open', 'Won', 'Lost'] as ResultFilter[]).map((f, i) => {
              ...
              const borderLeft = i > 0 ? 'border-l border-border' : '';
              const activeColor = f === 'Won' ? 'bg-pos-muted text-pos'
                : f === 'Lost' ? 'bg-neg-muted text-neg'
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 transition-colors ${borderLeft} ${
                    active ? activeColor : 'text-fg-subtle hover:text-fg-muted'
                  }`}
                >
                  {label}
                </button>
        ...
        {!loading && !error && grades.length > 0 && (
          <>
            <select
              ...
              className="bg-surface border border-border text-fg-muted text-xs rounded px-2 py-1 focus:outline-none focus:border-border-strong"
            >
            ...
            <input
              ...
              className="bg-surface border border-border text-fg-muted text-xs rounded px-2 py-1 w-28 focus:outline-none focus:border-border-strong placeholder-fg-disabled"
            />
          </>
        )}
        ...
        {!loading && !error && grades.length > 0 && (
          <div className="flex items-center gap-1.5">
            <select
              ...
              className={`bg-surface border text-xs rounded px-2 py-1 focus:outline-none focus:border-border-strong ${
                signalFilter ? 'border-brand text-brand' : 'border-border text-fg-muted'
              }`}
            >
            ...
            <button
              ...
              className="text-fg-disabled hover:text-fg-subtle text-xs"
              title="Signal legend"
            >
              ?
            </button>
        ...
        {/* View toggle */}
        {!loading && !error && grades.length > 0 && (
          <div className="flex rounded border border-border overflow-hidden text-xs font-medium ml-auto">
            <button
              onClick={() => setViewMode('list')}
              className={`px-3 py-1 transition-colors ${
                viewMode === 'list' ? 'bg-surface-hover text-fg' : 'text-fg-subtle hover:text-fg-muted'
              }`}
            >
              List
            </button>
            <button
              ...
              className={`px-3 py-1 transition-colors border-l border-border ${
                viewMode === 'matrix' ? 'bg-surface-hover text-fg' : 'text-fg-subtle hover:text-fg-muted'
              }`}
            >
              Matrix
            </button>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded px-2.5 py-1 text-xs transition-colors border whitespace-nowrap ${
                    isActive
                      ? 'border-brand bg-brand-muted text-fg'
                      : 'border-border hover:border-border-strong text-fg-subtle hover:text-fg-muted'
                  }`}
                >
                  <span className={awayWin ? 'font-semibold text-fg' : isFinal ? 'text-fg-disabled' : ''}>
                    {g.awayTeamAbbr}
                  </span>
                  {!isUpcoming && (
                    <span className={`tabular-nums font-semibold ${awayWin ? 'text-fg' : isFinal ? 'text-fg-disabled' : 'text-fg-muted'}`}>
                      {g.awayScore}
                    </span>
                  )}
                  <span className="text-fg-disabled">&#8211;</span>
                  {!isUpcoming && (
                    <span className={`tabular-nums font-semibold ${homeWin ? 'text-fg' : isFinal ? 'text-fg-disabled' : 'text-fg-muted'}`}>
                      {g.homeScore}
                    </span>
                  )}
                  <span className={homeWin ? 'font-semibold text-fg' : isFinal ? 'text-fg-disabled' : ''}>
                    {g.homeTeamAbbr}
                  </span>
                  {statusText && (
                    <span className={`ml-0.5 ${isLive ? 'text-pos' : isFinal ? 'text-fg-disabled' : 'text-fg-disabled'}`}>
      ...
      {!loading && !error && grades.length > 0 && (
        <div className="px-4 py-2 border-b border-border flex items-center gap-3">
          <span className="text-xs text-fg-disabled whitespace-nowrap">Min odds</span>
          <input
            type="range"
            min={ODDS_MIN}
            max={oddsMax}
          ...
          />
          <span className={`text-xs tabular-nums w-14 text-right ${oddsFilterActive ? 'text-fg-muted' : 'text-fg-disabled'}`}>
            {minOdds >= 0 ? `+${minOdds}` : `${minOdds}`}
          </span>
          {oddsFilterActive && (
            <button onClick={() => setMinOdds(ODDS_MIN)} className="text-xs text-fg-disabled hover:text-fg-subtle">
              Reset
            </button>
          )}
        </div>
      ...
      <div className="flex-1 px-4 py-4">
        {loading && <div className="text-sm text-fg-subtle">Loading grades...</div>}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-fg-subtle border-b border-border">
                  ...
                  <th className="text-right py-1.5 px-2 font-medium text-fg-subtle text-xs" title="Implied probability from odds">Imp%</th>
                ...
                {sorted.map((row) => {
                  ...
                  const oddsContent = isLive && displayPrice != null && priceChanged ? (
                    <span className="inline-flex items-center gap-1">
                      <span className="text-fg-disabled text-xs tabular-nums line-through">{fmtOdds(openingPrice)}</span>
                      <span className={priceRise ? 'text-pos' : 'text-warn'}>
                        {priceRise ? '↓' : '↑'}{fmtOdds(displayPrice)}
                      </span>
                    </span>
                  ) : (
                    <span className={isLive ? 'text-pos' : oddsColor(displayPrice)}>
                      ...
                      >
                        <td className="py-1.5 pr-3">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {(() => {
                              const playerTeam =
                                row.oppTeamAbbr && row.homeTeamAbbr && row.awayTeamAbbr
                                  ...
                                  : null;
                              return playerTeam ? (
                                <span className="text-[10px] font-medium tracking-wide text-fg-subtle tabular-nums">
                                  {playerTeam}
                                </span>
                              ) : null;
                            })()}
                            ...
                            {row.outcome === 'Won' && (
                              <span className="text-xs font-medium text-pos bg-pos-muted px-1 rounded">W</span>
                            )}
                            {row.outcome === 'Lost' && (
                              <span className="text-xs font-medium text-neg bg-neg-muted px-1 rounded">L</span>
                            )}
                            {isLive && (
                        ...
                        <td className="py-1.5 pr-1 text-fg-subtle text-xs font-mono">{marketAbbr(row.marketKey)}</td>

                        <td className="py-1.5 px-2 text-right text-fg-muted tabular-nums">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {liveStat !== null && (
                              <span className={`text-xs font-semibold tabular-nums ${
                                liveStat > row.lineValue ? 'text-pos' : 'text-neg'
                              }`}>
                                ({liveStat})
                              </span>
                            )}
                        ...
                        <td className="py-1.5 px-2 text-right tabular-nums text-fg-subtle text-xs">
                          {priceChanged
                        ...
                        <td className="py-1.5 px-2 text-right text-fg-muted">{fmtPct(row.hitRate20)}</td>
                        <td className="py-1.5 px-2 text-right text-fg-muted">{fmtPct(row.hitRate60)}</td>
                        <td
                          className={`py-1.5 px-2 text-right tabular-nums ${
                            row.hitRateOpp != null ? gradeColor(row.hitRateOpp * 100) : 'text-fg-disabled'
                          }`}
                          title={oppTitle}
                        >
                          {oppPct}
                          {row.sampleSizeOpp != null && (
                            <span className="text-fg-disabled text-xs ml-0.5">({row.sampleSizeOpp})</span>
                          )}
                        </td>
                        <td className="py-1.5 px-2 text-right text-fg-subtle">{row.sampleSize20 ?? '-'}</td>
                        <td className="py-1.5 px-2 text-right text-fg-subtle">{row.sampleSize60 ?? '-'}</td>
                        <td className={`py-1.5 pl-2 text-right tabular-nums text-xs ${
                          def.rank != null ? rankColor(def.rank) : 'text-fg-disabled'
                        }`}>{def.label}</td>
                      </tr>

                      {isExpanded && (
                        <tr key={`${rowKey}-exp`} className="border-b border-border bg-surface-hover">
                            ...
                            {livePlayer ? (
                              <div className="flex flex-wrap gap-4 text-xs">
                                {[
                                  ...
                                  <div key={label} className="text-fg-subtle">
                                    <span className="text-fg-disabled mr-1">{label}</span>
                                ...
                                {livePlayer.oncourt && (
                                  <span className="text-pos text-xs">&#9679; on court</span>
                                )}
                              </div>
                            ) : (
