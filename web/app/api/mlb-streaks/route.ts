...
ranked AS (
  ...
  -- Strictly BEFORE the slate date (pre-game state, same rule as the props
  -- join and the player_patterns readers — never bake in a row already
  -- computed through the slate day's own game). Also within 14 days: excludes
  -- inactive players whose most-recent state is seasons old (stale "27-game
  -- drought" from last September) and bounds the ranked set for speed.
  WHERE s.as_of_date < @d AND s.as_of_date >= @recent
)
SELECT
