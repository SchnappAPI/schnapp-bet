...
import { fetchMlbLiveOverlay, type MlbLiveOverlay } from "@/lib/mlbLive";
...
function nbaStatus(g: GameRow): Status {
  ...
  if (g.gameStatusText && /ppd|postponed/i.test(g.gameStatusText))
    return "postponed";
  return "scheduled";
}
...
function mlbStatus(s: string | null): Status {
  ...
  if (u === "I" || u === "L" || u === "LIVE" || u === "IN PROGRESS")
    return "live";
  return "scheduled";
}
...
async function fetchNbaGames(
  date: string,
): Promise<{ count: number; games: SportGame[] }> {
...
async function fetchMlbGames(
  ...
  const games: SportGame[] = result.recordset.map((g) => {
    const o = liveByGamePk.get(Number(g.gameId));
    const mergedStatusStr = o?.gameStatus ?? g.gameStatus;
    const status: Status =
      o?.abstractState === "Live" ? "live" : mlbStatus(mergedStatusStr);
    const awayScore = o?.awayScore ?? g.awayScore;
    const homeScore = o?.homeScore ?? g.homeScore;
    const tipoff = g.gameDateTime
      ? g.gameDateTime instanceof Date
        ? g.gameDateTime.toISOString()
        : new Date(g.gameDateTime).toISOString()
      : null;
