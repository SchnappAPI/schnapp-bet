...
All workflows and the production web tier (`bet.schnapp.web-prod`) target the local SQL Server container on Schnapps-MBP (`localhost,1433`, SA credentials in `/Users/schnapp/sql-server.env`, `TrustServerCertificate=yes`).
...
The local SQL container does not auto-pause. `keepalive.yml` is dispatch-only and should not be rescheduled without making a deliberate decision to reverse the tradeoff.
...
Git log is the changelog (ADR-20260517-4): `git log --grep='\[nba\]' --grep='\[database\]'`.
