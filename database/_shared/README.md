...
`bootstrap.sql` in this directory is a generated full-schema snapshot of the `common` and `odds` schemas (regenerate via `/skill regenerate-bootstrap-sql`; never hand-edit). Runtime schema evolution for the integrity meta tables is owned by `shared/integrity.py` (`DDL_STATEMENTS` + `ensure_tables()`).
...
- Data-integrity framework tables (`common.ingest_quarantine`, `common.unmapped_entities`, `common.data_completeness_log`) are owned by `shared/integrity.py`. Do not write directly to them from ETL scripts — always go through `validate_and_filter()`, `record_unmapped_entity()`, or `retroactive_scan()`. The framework tables are the state for Layers 1-3 of the integrity design (ADR-20260424-2); direct writes bypass the invariants.
- `common.*` tables opted into CRITICAL_FIELDS enforcement as of 2026-04-24: `teams`, `user_codes`, `demo_config`, `player_line_patterns`, `daily_grades`, `player_tier_lines`. Catalog lives in `shared/integrity.py`. `common.user_activations` is not yet in the catalog (add if and when a write path touches it from the ETL).

## Recent Changes

Git log is the changelog (ADR-20260517-4): `git log --grep='\[shared\]' --grep='\[database\]'`.
