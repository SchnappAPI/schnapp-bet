...
- **MLB in-season pipeline.** Nightly PBP + grading at 16 markets (analytic tier engine `mlb-v1.2`, ADR-20260712-2), batter context + projections, `/mlb/grades` surface. Verify end-to-end once odds flow again.
- **NFL — live corpus.** `nfl_grade_props.py` (`nfl-v1.0`) + `nfl-grading.yml` are shipped with a clean offseason exit; live odds corpus accrues once the Odds API key is restored and the September season starts. Design contract in ADR-20260703-2.

## Next

- **grading-v2 Phases 8–9 decision (NBA).** Backend emits `model_prob`/`ev_pct`/`player_value_lines` daily; the web still renders the composite grade. Ship the web display + historical re-grade, or formally shelve. (The `grading-v2/` scratch tree was removed 2026-07-15; its history is in git.)
...
- **MLB pattern quality monitoring.** `mlb.player_patterns` populates nightly; the monitoring shape is an open owner question (`docs/features/mlb-research-dashboard-remainder.md` Q4).
