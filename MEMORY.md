...
**MLB research dashboard Phases 2+3 shipped (2026-07-04, Mac session).** API: `/api/mlb/research/{slate,grid,atbats}` — grid aggregates `player_game_statcast` raw sums (avgEv = SUM(ev_sum)/SUM(bbe)); hand/abNum filters drop to `player_at_bats` grain with ETL-lockstep event definitions (`web/lib/mlbResearch.ts`). Two data-shape gotchas now encoded: `hit_probability` is 0-100 (all xBA display goes through `statcastFormat.fmtXba` — three surfaces were rendering it raw), and `at_bat_number` is the GAME-wide PA sequence, so the 1-6 AB slicer uses ROW_NUMBER per (batter, game). UI: `/mlb/research` (slicer row → both-lineup heat grid → batter detail), `HeatCell` + `web/lib/colorScale.ts` percentile shading, sidebar link; live Exit Velo tab retrofitted to the same scale. **`MlbEvView`/`MlbVsView`/`MlbProjView` are orphaned** (retired 6-tab page, nothing imports them) — retrofitted per plan but they are deletion candidates. Phase 1 leftovers done: mlb bootstrap.sql regenerated; Savant spot-check passed (avg/max EV exact for Cruz/Wood/Alvarez, hard-hit within 0.8pt; barrel% diverges by design — proxy definition D3). Next: Phase 4 (patterns table + hit_prob/hr_prob markets). Not deployed — owner `/deploy` when ready.
