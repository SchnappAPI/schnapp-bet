...
def _migrate_players_schema(engine):
    with engine.begin() as conn:
        conn.execute(text("""
            IF NOT EXISTS (
                SELECT 1 FROM sys.columns
                WHERE object_id = OBJECT_ID('mlb.players') AND name = 'last_seen_season'
            )
                ALTER TABLE mlb.players ADD last_seen_season INT NOT NULL DEFAULT 0;
        """))


def load_players(engine, seasons):
    # Dict keyed by player_id; last season processed wins for dedup so each
    # player retains their most recent season's team_id, position, etc.
    players = {}
    ...
    log.info("Merging %d unique players across seasons %s", len(records), seasons)
    merge_sql = text("""
        MERGE mlb.players AS tgt
        USING (VALUES (
            :player_id, :player_name, :team_id, :position,
            :bat_side, :pitch_hand, :last_seen_season
        )) AS src (player_id, player_name, team_id, position,
                   bat_side, pitch_hand, last_seen_season)
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                bat_side         = src.bat_side,
                pitch_hand       = src.pitch_hand,
                last_seen_season = CASE
                    WHEN src.last_seen_season > tgt.last_seen_season
                    THEN src.last_seen_season
                    ELSE tgt.last_seen_season
                END
        WHEN NOT MATCHED THEN
            INSERT (player_id, player_name, team_id, position,
                    bat_side, pitch_hand, last_seen_season)
            VALUES (src.player_id, src.player_name, src.team_id, src.position,
                    src.bat_side, src.pitch_hand, src.last_seen_season);
    """)
    with engine.begin() as conn:
        conn.execute(merge_sql, records)
    log.info("  %d player rows merged", len(records))
...
def main():
    ...
    _migrate_players_schema(engine)
