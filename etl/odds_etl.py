...
def run_upcoming(sport, api_key, days_ahead, engine):
    ...
    for event in in_window:
        ...
        _ue_rows = validate_and_filter([{
            ...
            "away_tricode":  away_tc,
            "game_id":       game_id,
        }], "odds.upcoming_events", engine, "odds-etl.yml")
        if _ue_rows:
            upsert(engine, clean_dataframe(pd.DataFrame(_ue_rows)),
                   schema="odds", table="upcoming_events", keys=["event_id"])

        _egm_rows = validate_and_filter([{
            "event_id":     eid,
            "sport_key":    sport_key,
            "game_id":      game_id,
        ...
        }], "odds.event_game_map", engine, "odds-etl.yml")
        if _egm_rows:
            upsert(engine, clean_dataframe(pd.DataFrame(_egm_rows)),
                   schema="odds", table="event_game_map", keys=["event_id"])
        ...
        if pp_all:
            ...
            if "link" not in pp_df.columns:
                pp_df["link"] = None
            pp_rows = validate_and_filter(
                pp_df.to_dict(orient="records"),
                "odds.upcoming_player_props", engine, "odds-etl.yml"
            )
            if pp_rows:
                upsert(engine, clean_dataframe(pd.DataFrame(pp_rows)),
                       schema="odds", table="upcoming_player_props",
                       keys=["event_id", "market_key", "bookmaker_key", "player_name", "outcome_name"])
            pp_total += len(pp_rows) if pp_rows else 0
