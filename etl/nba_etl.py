...
Secrets required
  NBA_PROXY_URL, SQL_SERVER, SQL_DATABASE,
...
DDL_STATEMENTS = [
    ...
    """
    IF NOT EXISTS (SELECT 1 FROM sys.objects
                   WHERE object_id = OBJECT_ID(N'nba.player_usage_stats') AND type = 'U')
    CREATE TABLE nba.player_usage_stats (
        ...
        team_tricode        CHAR(3)      NULL,
        minutes             FLOAT        NULL,
        usage_pct           FLOAT        NULL,
        est_usage_pct       FLOAT        NULL,
        pace                FLOAT        NULL,
        pace_per40          FLOAT        NULL,
        possessions         INT          NULL,
        created_at          DATETIME2    NOT NULL DEFAULT GETUTCDATE(),
        CONSTRAINT pk_player_usage_stats PRIMARY KEY (usage_id),
        CONSTRAINT uq_player_usage_stats UNIQUE (game_id, player_id)
    )
    """,
]
...
def load_schedule(engine, season):
    ...
    if schedule_rows:
        schedule_rows = validate_and_filter(schedule_rows, "nba.schedule", engine, "nba-etl.yml")
        upsert(pd.DataFrame(schedule_rows), engine, "nba", "schedule", ["game_id"])
        log.info(f"  {len(schedule_rows)} schedule rows upserted")
    if games_rows:
        games_rows = validate_and_filter(games_rows, "nba.games", engine, "nba-etl.yml")
...
def fetch_and_upsert_box_scores(date_from, season, engine):
    ...
    for game_date in sorted(rows_by_date):
        date_rows = validate_and_filter(
            rows_by_date[game_date], "nba.player_box_score_stats", engine, "nba-etl.yml"
        )
        if not date_rows:
            continue
...
def load_passing_stats(game_date, season, engine, known_player_ids):
    ...
    if rows:
        rows = validate_and_filter(rows, "nba.player_passing_stats", engine, "nba-etl.yml")
...
def load_rebound_chances(game_date, season, engine, known_player_ids):
    ...
    if skipped:
        log.warning(f"  Rebound chances: skipped {len(skipped)} unknown player(s) for {game_date}: "
                    + ", ".join(f"{name} ({pid})" for pid, name in skipped))
    if rows:
        rows = validate_and_filter(rows, "nba.player_rebound_chances", engine, "nba-etl.yml")
...
def load_daily_lineups(schedule_today, engine):
    ...
    for game_date, group in groupby(to_fetch, key=lambda r: r["game_date"]):
        ...
        if filtered:
            filtered = validate_and_filter(filtered, "nba.daily_lineups", engine, "nba-etl.yml")
        if filtered:
            with engine.begin() as conn:
...
# Main
# ---------------------------------------------------------------------------
def _parse_iso_minutes(raw):
    """Parse ISO 8601 duration string like PT26M43.00S into decimal minutes."""
    if not raw:
        return None
    try:
        s = str(raw).replace("PT", "")
        mins = 0.0
        if "M" in s:
            parts = s.split("M")
            mins = float(parts[0])
            s = parts[1]
        if "S" in s:
            mins += float(s.replace("S", "")) / 60.0
        return round(mins, 4)
    except Exception:
        return None


def _parse_advanced_boxscore(data, game_id, game_date):
    """Extract per-player usage stats from boxscoreadvancedv3 response."""
    rows = []
    try:
        for side in ("homeTeam", "awayTeam"):
            team_data = data.get("boxScoreAdvanced", {}).get(side, {})
            team_id = safe_int(team_data.get("teamId"))
            team_tricode = safe_str(team_data.get("teamTricode"))
            for p in team_data.get("players", []):
                pid = safe_int(p.get("personId"))
                if pid is None:
                    continue
                ...
                rows.append({
                    ...
                    "game_date":     game_date,
                    "player_id":     pid,
                    "player_name":   safe_str(p.get("name")),
                    "team_id":       team_id,
                    "team_tricode":  team_tricode,
                    "minutes":       _parse_iso_minutes(stats.get("minutes")),
                    "usage_pct":     safe_float(stats.get("usagePercentage")),
                    "est_usage_pct": safe_float(stats.get("estimatedUsagePercentage")),
                    "pace":          safe_float(stats.get("pace")),
                    "pace_per40":    safe_float(stats.get("pacePer40")),
                    "possessions":   safe_int(stats.get("possessions")),
                })
    except Exception as exc:
        log.warning(f"  _parse_advanced_boxscore({game_id}): {exc}")
    return rows


def load_player_usage_stats(engine, days=10):
    """
    Fetch boxscoreadvancedv3 for completed games missing from nba.player_usage_stats.
    Processes up to `days` game dates per run to stay within timeout limits.
    """
    with engine.connect() as conn:
        missing_games = [
            (str(r[0]), r[1])
            for r in conn.execute(text("""
                SELECT TOP 200 g.game_id, g.game_date
                FROM nba.games g
                WHERE g.game_status = 3
                  AND NOT EXISTS (
                      SELECT 1 FROM nba.player_usage_stats u
    ...
    if not missing_games:
        log.info("Usage stats: all games up to date.")
        return

    # Limit by distinct dates
    seen_dates = []
    filtered = []
    for gid, gdate in missing_games:
        d = str(gdate)
        if d not in seen_dates:
            if len(seen_dates) >= days:
                break
            seen_dates.append(d)
        filtered.append((gid, gdate))

    log.info(f"Usage stats: {len(filtered)} games across {len(seen_dates)} date(s).")

    total = 0
    for gid, gdate in filtered:
        data = _direct_get(
            "https://stats.nba.com/stats/boxscoreadvancedv3",
            f"boxscoreadvancedv3 {gid}",
            params={"GameID": gid, "LeagueID": "00",
                    "endPeriod": 0, "endRange": 0, "rangeType": 0,
                    "startPeriod": 0, "startRange": 0},
            proxies=get_proxies(), timeout=60,
        )
        if data is None:
            log.warning(f"  Usage stats: no response for {gid}")
            continue

        rows = _parse_advanced_boxscore(data, gid, gdate)
        if not rows:
            log.warning(f"  Usage stats: 0 players parsed for {gid}")
            continue

        df = pd.DataFrame(rows)
        upsert(df, engine, "nba", "player_usage_stats", ["game_id", "player_id"])
        log.info(f"  Usage stats: {len(rows)} players upserted for {gid} ({gdate})")
        total += len(rows)
        time.sleep(1)  # polite delay between game calls

    log.info(f"Usage stats phase complete. {total} total rows.")
...
def main():
    ...
    # Usage stats for grading-v2 relevance weighting
    load_player_usage_stats(engine, days=args.days)

    log.info("NBA ETL complete.")


# ---------------------------------------------------------------------------
# Targeted per-game-id fetches (used by retry-incomplete.yml)
# ---------------------------------------------------------------------------
...
def fetch_scoreboard_games(game_id, engine):
    """
    Fetch a single nba.schedule row for game_id via boxscoresummaryv2.
    Returns a schedule-shaped dict or None on failure.
    """
    data = _direct_get(
        "https://stats.nba.com/stats/boxscoresummaryv2",
        f"boxscoresummaryv2 {game_id}",
    ...
    if data is None:
        return None
    try:
        summary_df   = _parse_result_set(data, index=0)  # GameSummary
        linescore_df = _parse_result_set(data, index=5)  # LineScore
        if summary_df is None or summary_df.empty:
            return None
        s = summary_df.iloc[0]

        home_id      = safe_int(s.get("HOME_TEAM_ID"))
        away_id      = safe_int(s.get("VISITOR_TEAM_ID"))
        home_score   = home_tricode = away_score = away_tricode = None

        if linescore_df is not None and not linescore_df.empty:
            for _, ls in linescore_df.iterrows():
                tid = safe_int(ls.get("TEAM_ID"))
                if tid == home_id:
                    home_score   = safe_int(ls.get("PTS"))
                    home_tricode = safe_str(ls.get("TEAM_ABBREVIATION"))
                elif tid == away_id:
                    away_score   = safe_int(ls.get("PTS"))
                    away_tricode = safe_str(ls.get("TEAM_ABBREVIATION"))

        return {
            "game_id":           game_id,
            "game_date":         safe_date(s.get("GAME_DATE_EST")),
            "season_type":       None,
            "game_code":         safe_str(s.get("GAMECODE")),
            "game_status":       safe_int(s.get("GAME_STATUS_ID")),
            "game_status_text":  safe_str(s.get("GAME_STATUS_TEXT")),
            "home_team_id":      home_id,
        ...
        log.warning(f"  fetch_scoreboard_games({game_id}) parse failed: {exc}")
        return None


def fetch_boxscore(game_id, engine):
    """
    Fetch nba.player_box_score_stats rows for a single game via playergamelogs.
    Returns a list of row dicts (one per player×period) or None on failure.
    Looks up game_date from nba.schedule to scope the playergamelogs call.
    """
    try:
        with engine.connect() as conn:
            sched = conn.execute(
                text("SELECT game_date FROM nba.schedule WHERE game_id = :gid"),
                {"gid": game_id},
            ).fetchone()
    except Exception:
        sched = None
    ...
    if sched is None:
        log.warning(f"  fetch_boxscore: game_id {game_id} not in nba.schedule")
        return None
    ...
    for period_value, game_segment, period_label in PERIOD_CONFIG:
        df = _fetch_playergamelogs_from(period_value, game_segment, period_label, game_date, season)
        if df is None:
            continue
        for _, r in df.iterrows():
            if safe_str(r.get("GAME_ID")) != game_id:
                continue
            pid      = safe_int(r.get("PLAYER_ID"))
            row_date = safe_date(r.get("GAME_DATE"))
            if pid is None or row_date is None:
                continue
            rows.append({
                "game_id":        game_id,
                "player_id":      pid,
                "period":         period_label,
                "season_year":    safe_str(r.get("SEASON_YEAR")),
                "player_name":    safe_str(r.get("PLAYER_NAME")),
                "team_id":        safe_int(r.get("TEAM_ID")),
                "team_tricode":   safe_str(r.get("TEAM_ABBREVIATION")),
                "game_date":      row_date,
                "matchup":        safe_str(r.get("MATCHUP")),
                "minutes":        safe_float(r.get("MIN")),
                "minutes_sec":    safe_str(r.get("MIN_SEC")),
                "fgm":            safe_int(r.get("FGM")),
                "fga":            safe_int(r.get("FGA")),
                "fg_pct":         safe_float(r.get("FG_PCT")),
                "fg3m":           safe_int(r.get("FG3M")),
                "fg3a":           safe_int(r.get("FG3A")),
                "fg3_pct":        safe_float(r.get("FG3_PCT")),
                "ftm":            safe_int(r.get("FTM")),
                "fta":            safe_int(r.get("FTA")),
                "ft_pct":         safe_float(r.get("FT_PCT")),
                "oreb":           safe_int(r.get("OREB")),
                "dreb":           safe_int(r.get("DREB")),
                "reb":            safe_int(r.get("REB")),
                "ast":            safe_int(r.get("AST")),
                "tov":            safe_int(r.get("TOV")),
                "stl":            safe_int(r.get("STL")),
                "blk":            safe_int(r.get("BLK")),
                "blka":           safe_int(r.get("BLKA")),
                "pf":             safe_int(r.get("PF")),
                "pfd":            safe_int(r.get("PFD")),
                "pts":            safe_int(r.get("PTS")),
                "plus_minus":     safe_int(r.get("PLUS_MINUS")),
                "dd2":            safe_int(r.get("DD2")),
                "td3":            safe_int(r.get("TD3")),
                "available_flag": safe_int(r.get("AVAILABLE_FLAG")),
            })
