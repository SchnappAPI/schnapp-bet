...
- CHANGELOG: one file per year at `docs/changelog/YYYY.md`. Newest entries at the top, grouped under `## YYYY-MM-DD` headers. One-liner per logical change: `- [scope][component] short description — ADR-YYYYMMDD-N` (ADR ref optional). The CHANGELOG entry rides in the same commit as the change (ADR-20260517-3). Long-form context lives in the ADR or commit message, not the CHANGELOG. Tag taxonomy and full spec: `docs/changelog/README.md`.
